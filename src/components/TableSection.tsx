...
export const TableSection: React.FC<TableSectionProps> = React.memo(({
  ...
  // Convert existing data format to match the new format
  const convertedData = useMemo(() => {
    const allRows = [...data.coefficients];
    
    // Add intercept if it exists
    if (data.intercept) {
      allRows.unshift({
        variable: 'Intercept',
        coefficient: data.intercept.coefficient,
        standardError: data.intercept.standardError,
        tStatistic: data.intercept.tStatistic,
        pValue: data.intercept.pValue,
        significance: data.intercept.significance
      });
    }
    ...
    return allRows.map(row => ({
      ...
      std_err: row.standardError,
      t: row.tStatistic,
      p_value: row.pValue,
      ci_lower: row.coefficient - 1.96 * row.standardError, // Approximate 95% CI
      ci_upper: row.coefficient + 1.96 * row.standardError
    }));
  }, [data.coefficients, data.intercept]);
  ...
  const tableRows = useMemo(() => 
    ...
    )), [convertedData, visibleColumnOrder, formatNumber, getSignificanceStars]
  );
